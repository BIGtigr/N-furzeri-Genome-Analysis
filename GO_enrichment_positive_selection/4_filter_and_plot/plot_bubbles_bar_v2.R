...
# This is to plot both gene number as bubble size and enrichment as heatmap
...
# This is to plot bubbles of same size and only enrichment as heatmap
p + geom_point(aes(colour = log(bar$enrichment), size = 3)) + 
  scale_colour_gradient(low = "blue", high = "red") + 
  scale_size(range = c(3, 9)) +
  scale_y_discrete(label = rev(bar$Term)) # for Go Id it would be bar$GOBPID 
